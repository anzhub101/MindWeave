...
export function InspectorDrawer({ node, documents, onClose }: InspectorDrawerProps) {
  const snippets = documents
    .filter((document) => node.evidence_refs.includes(document.id))
    .map((document) => ({
      id: document.id,
      name: document.name,
  ...
  return (
    <aside className="flex min-h-0 flex-col overflow-hidden border-l border-[var(--mw-border)] bg-[var(--mw-page)]">
